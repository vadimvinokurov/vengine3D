...
std::vector<VE::Mesh> VE::GLTF::loadMeshes() {
    ...
    tcb::span<cgltf_node> nodes(data_->nodes, data_->nodes_count);
    for (const auto &node: nodes) {
        ...
        for (const auto &primitive: primitives) {
            ...
            for (const auto &attribute: attributes) {
                meshFromAttribute(mesh, attribute, node.skin, data_->nodes, nodes.size());
            }

            if (primitive.indices != 0) {
                mesh.indices.reserve(primitive.indices->count);
                mesh.indices.clear();
                for (std::size_t k = 0; k < primitive.indices->count; ++k) {
                    mesh.indices.emplace_back(cgltf_accessor_read_index(primitive.indices, k));
                }
            }
...
VE::Pose VE::GLTF::loadBindPose() {
    ...
    for (std::size_t i = 0; i < skinsCount; ++i) {
        ...
        for (std::size_t j = 0; j < numSkinJoints; ++j) {
            auto jointIndex = getNodeIndex(skin.joints[j], data_->nodes, data_->nodes_count);
...
void
VE::GLTF::meshFromAttribute(VE::Mesh &mesh, const cgltf_attribute &attribute, cgltf_skin *skin, cgltf_node *allNodes,
    ...
    switch (attributeType) {
        ...
        case cgltf_attribute_type_joints: {
            ...
            for (std::size_t i = 0; i < jointsIndexFloat.size(); i++) {
                IVector4 joint(jointsIndexFloat[i].x + 0.5f, jointsIndexFloat[i].y + 0.5f, jointsIndexFloat[i].z + 0.5f,
                               jointsIndexFloat[i].w + 0.5f);
