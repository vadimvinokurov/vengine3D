...
void ContactSolver::preStep(float dt) {
    for (ManifoldContactPoint &contact: contactMainfold_) {
        ...
        Vector nCrossR1 = r1 * contact.normal * -1;
        Vector nCrossR2 = r2 * contact.normal;
        contact.normalEffectiveMass = 1 / (body1.invMass() + body2.invMass() + (body1.invInertia()*nCrossR1).dot(nCrossR1) + (body2.invInertia()*nCrossR2).dot(nCrossR2));
        contact.normalInitRelativeVelocity = std::min(relativeVelocity.dot(contact.normal) + speedSlop, 0.0f);
        float ERP = -beta / dt;
        contact.bias = ERP * std::max(0.0f, contact.collisionDepth - penetrationSlop);
    }
}
...
void ContactSolver::applyImpulse(float dt) {
    for (ManifoldContactPoint &contactPoint: contactMainfold_) {
        ...
        float dPn = contactPoint.normalEffectiveMass * -(vn + e * contactPoint.normalInitRelativeVelocity + contactPoint.bias);
...
//        for (ContactPoint &contactPoint: contactMainfold_) {
//            Vector r1 = contactPoint.point - body1.centerMass();
//            Vector r2 = contactPoint.point - body2.centerMass();
//
//            Vector vrel = body2.pseudoLinearVelocity() + body2.pseudoAngularVelocity() * r2 - body1.pseudoLinearVelocity() - body1.pseudoAngularVelocity() * r1;
//
//            float vn = vrel.dot(contactPoint.normal);
//
...
//            body1.setPseudoAngularVelocity(body1.pseudoAngularVelocity() - (r1 * L) * body1.invInertia());
//
//            body2.setPseudoLinearVelocity(body2.pseudoLinearVelocity() + L * body2.invMass());
//            body2.setPseudoAngularVelocity(body2.pseudoAngularVelocity() + (r2 * L) * body2.invInertia());
//        }
//    }
//}
