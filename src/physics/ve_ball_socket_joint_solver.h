...
namespace VE {
    ...
    public:
        ...
        void applyImpulse(float dt) {
            ...
            VE::Matrix4 Jw1(0.0f, r1.z, -r1.y,
                             -r1.z, 0.0f, r1.x,
                             r1.y, -r1.x, 0.0f);
            VE::Matrix4 Jw2(0.0f, -r2.z, r2.y,
                             r2.z, 0.0f, -r2.x,
                             -r2.y, r2.x, 0.0f);
            Jw1.transpose();
            Jw2.transpose();

            VE::Matrix4 k = Jv1 * body1->invMass() * Jv1.getTransposed() + Jw1 * body1->invInertia() * Jw1.getTransposed() +
                             Jv2 * body2->invMass() * Jv2.getTransposed() + Jw2 * body2->invInertia() * Jw2.getTransposed();
            VE::Matrix4 effectiveMass = k.getInversed();
