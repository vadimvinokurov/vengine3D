...
void World::resetScene() {
	...
	actors_.clear();
	scene();
	for (auto& actor : actors_) { std::copy(actor->getObjects().begin(), actor->getObjects().end(), std::back_inserter(worldObjects)); }
}
...
void World::hid(float dt) {
	if (mouse_->isPressed(VE_MOUSE_BUTTON_1)) {
		...
		if (minLen != std::numeric_limits<float>::max()) { mouseJointSolver_ = std::make_shared<VE::MouseJointSolver>(selectObject, selectObject->centerOfMass()); }
	}
	if (mouse_->isRelease(VE_MOUSE_BUTTON_1)) { mouseJointSolver_.reset(); }
	if (keyboard_->isPressed(VE_KEY_F1)) { globalParameters.polygone = !globalParameters.polygone; }
	cameraControl(dt);
}
...
void World::cameraControl(float dt) {
	...
	if (mouse_->isPressed(VE_MOUSE_BUTTON_2)) { mouse_->lockMouse(); }
	if (mouse_->isRelease(VE_MOUSE_BUTTON_2)) { mouse_->unlockMouse(); }
	if (mouse_->isRepeat(VE_MOUSE_BUTTON_2)) { currentCamera_->setDirection(mouse_->deltaPosition().y, mouse_->deltaPosition().x * -1); }

	if (keyboard_->isRepeat(VE_KEY_W)) { currentCamera_->moveAlongDirection(cameraSpeed); }
	if (keyboard_->isRepeat(VE_KEY_S)) { currentCamera_->moveAlongDirection(-cameraSpeed); }
	if (keyboard_->isRepeat(VE_KEY_D)) { currentCamera_->moveAlongSide(cameraSpeed); }
	if (keyboard_->isRepeat(VE_KEY_A)) { currentCamera_->moveAlongSide(-cameraSpeed); }
}
...
void World::physics(float dt) {
	for (auto& object : worldObjects) { object->updateVelocity(dt); }

	for (int i = 0; i < globalParameters.iterations; i++) {
		if (mouseJointSolver_) { mouseJointSolver_->applyImpulse(dt, currentCamera_->getPointAlongDirection(10)); }
	}

	for (int i = 0; i < globalParameters.iterations; i++) {
		for (auto& actor : actors_) { actor->update(dt); }
	}

	for (auto& contact : contactSolvers) { contact.second.preStep(dt); }

	for (int i = 0; i < globalParameters.iterations; i++) {
		for (auto& contact : contactSolvers) { contact.second.applyImpulse(dt); }
	}

	for (int i = 0; i < globalParameters.iterations; i++) {
		for (auto& contact : contactSolvers) { contact.second.applyPseudoImpulse(dt); }
	}

	for (auto& object : worldObjects) { object->updateTransform(dt); }
}
